...
func (che *chExecutor) init(ctx context.Context,
	...
	chClient, err := channel.New(sdkComps.chProvider, opts...)
	if err != nil {
		return errors.WithStack(err)
	}
...
func (che *chExecutor) executeBatch(ctx context.Context, b *executordto.Batch) (uint64, error) {
	...
	if err != nil {
		return 0, errorshlp.WrapWithDetails(
	...
	now := time.Now()
	resp, err := che.executeWithRetry(ctx, batchBytes)
...
func (che *chExecutor) executeWithRetry(ctx context.Context, batchBytes []byte) (channel.Response, error) {
