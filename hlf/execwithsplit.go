...
func (hlp *execWithSplitHlp) execute(ctx context.Context, b *executordto.Batch) (uint64, error) {
	...
	for executedTxsCount < len(b.Txs) {
		tmpBatch := splitBatchForExec(executedTxsCount, lastSuccessCount, lastErrorCount, b)
		hlp.log.Infof("try to exec part of original batch, txs: %v executed: %v from %v", len(tmpBatch.Txs), executedTxsCount, len(b.Txs))
...
func splitBatchForExec(executedTxsCount, lastSuccessCount, lastErrorCount int, origBatch *executordto.Batch) *executordto.Batch {
	const lessK = 2
