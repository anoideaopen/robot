...
func waitSwap(ctx context.Context, u *wallet.User, ch, cc, swapID string) error {
	for {
		...
		_, err := u.QueryWithRetryIfEndorsementMismatch(ch, cc, "swapGet", swapID)
		if err == nil {
			return nil
		}
...
func waitMultiSwap(ctx context.Context, u *wallet.User, ch, cc, swapID string) error {
	for {
		...
		_, err := u.QueryWithRetryIfEndorsementMismatch(ch, cc, "multiSwapGet", swapID)
		if err == nil {
			return nil
		}
		time.Sleep(1 * time.Second)
...
func getAllowedBalance(_ context.Context, user *wallet.User, channelName, chainCodeName, currency string) (uint64, error) {
	tokenBalanceResponse, err := user.QueryWithRetryIfEndorsementMismatch(channelName, chainCodeName, "allowedBalanceOf", user.Addr(), strings.ToUpper(currency))
	if err != nil {
		return 0, err
	}
...
func getIndustrialBalance(_ context.Context, user *wallet.User, channelName, chainCodeName, group string) (uint64, error) {
	tokenBalanceResponse, err := user.QueryWithRetryIfEndorsementMismatch(channelName, chainCodeName, "industrialBalanceOf", user.Addr())
	if err != nil {
		return 0, errors.WithStack(err)
	}
...
func waitBalance(ctx context.Context, expectedBalance uint64, getBalance func() (uint64, error)) error {
	...
	return fmt.Errorf("didn't got expected balance: "+strconv.Itoa(int(expectedBalance))+" current balance is "+strconv.Itoa(int(b)), errors.WithStack(ctx.Err()))
}
...
func isRateSet(u *wallet.User, token, currency string, rateValue uint64) (bool, error) {
	...
	resp, err := u.QueryWithRetryIfEndorsementMismatch(token, token, "metadata")
	if err != nil {
